...
def audit_project(root=ROOT):
    ...
    # в дело, которое библиотека знает. documents.json — каталог и
    # очередь, поэтому его ID не обязаны быть скачаны.
    #
    # Папки на диске для этого мало. Сканы здесь кэш, `prune.py` их
    # выбрасывает штатно, а источником факта бывает и том, который мы
    # сознательно не держим: «Алфавитный список населенных мест» 1915
    # года (`bx0000206`) отсеян из очереди — имён в нём нет, — но строки
    # Кочетовской и Ровенек из него выписаны в `places.json`, и ссылка
    # на него верная. Пока проверялось только наличие папки, уборка
    # после пробы делала записанный источник «неизвестным документом».
    known = set(docs) | catalog_ids
