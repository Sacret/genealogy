...
/* Год, из которого фильтр не оставил ни одного дела. Рамка сплошная, а не
   пунктир, как у пробела: год смотрели, просто в выборку он не попал.
   Цвет итога снят — он говорил бы о делах, которых сейчас не видно. */
.year.off { background: transparent; color: var(--dim); border-color: var(--line);
            font-weight: normal; opacity: .4; pointer-events: none; }
...
function apply() {
  ...
  document.querySelectorAll('.year-block').forEach(b => {
    ...
    // к якорю, под которым фильтр ничего не оставил. Связь — через якорь,
    // стоящий прямо перед блоком, на него же ссылается пузырь.
    const mark = b.previousElementSibling;
    const cell = mark && mark.classList.contains('year-mark') &&
                 document.querySelector(`.years a[href="#${mark.id}"]`);
    if (cell) {
      cell.classList.toggle('off', !left);
      if (left) {
...
// Фильтр живёт в адресе — ?q=фамилия&s=ok,maybe, — чтобы выборку можно
// было переслать ссылкой и открыть ровно на ней. Пишется replaceState, а
// не pushState: каждая набранная буква иначе стала бы шагом «назад». Якорь
// дела (#bv0000386) при этом остаётся на месте — он живёт в hash, а не в
// параметрах.
function syncURL() {
  ...
  if (on.length) url.searchParams.set('s', on.join(','));
...
function restoreFromURL() {
  ...
  if (q) box.value = q;
  const s = (params.get('s') || '').split(',').filter(Boolean);
  // Итога, которого в журнале уже нет, среди чипов просто не найдётся —
  // старая ссылка откроется на том, что от её выборки осталось.
  chips.forEach(c => c.setAttribute('aria-pressed',
                                    s.includes(c.dataset.s) ? 'true' : 'false'));
  if (q || s.length) apply();
}
box.addEventListener('input', () => { apply(); syncURL(); });
...
// Якорь в адресе — это просьба показать дело или год, и при перезагрузке
// тоже. Сам браузер при перезагрузке якорь не слушает: он возвращает
// прокрутку, какая была, и страница открывалась где-то посередине, мимо
// дела из адреса. Поэтому восстановление снято — но только когда якорю
// есть куда вести, иначе перезагрузка без якоря бросала бы читающего
// наверх.
...
// Прокрутка дважды: сразу — чтобы не мелькнул верх страницы, и на load —
// когда догрузился шрифт и шапка перемерена, иначе дело съехало бы на её
// разницу. Если человек успел прокрутить сам, второй раз его не дёргаем.
let moved = false;
