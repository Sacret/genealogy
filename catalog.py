...
# Именная роспись сильнее тематического отсева. «Адрес-календарь
# служащих Владикавказской железной дороги» — это перечень людей с
# должностями и станциями, а правило о дороге судит по теме и увело бы
# его из очереди вместе с уставами и тарифами. Спрашивается раньше
# `SKIP`, и очередь называет сам: под `COSSACK` и `REGION` такой
# заголовок не подходит вовсе — в нём нет ни Дона, ни Ростова.
NAMED_LISTS = re.compile(r"Адрес-календарь служащих", re.I)
...
COSSACK = re.compile("|".join([
    ...
    # Роспись родов — это готовый именной указатель, даже когда станица
    # чужая: «Казаки станицы Луганской» под `Дон` не подходят вовсе.
    r"генеалоги\w+ (казачьих|донских) родов",
]), re.I)
...
SKIP_BY_ID = {
    ...
    "bv0000047": "публикация актов XVII века: люди названы по отчеству "
                 "(проверено пробой из шести страниц)",
    # Те же таблицы поселений, что в bv0000264 и bv0000265, только на
    # четверть века раньше. Проба из шести страниц вразброс: ни одного
    # личного имени на 11 тысяч знаков — двор, житель и голова скота
    # идут счётом. Похожее на фамилию — имена хуторов: «Матвѣевъ-
    # Сулинъ», «Калмыковъ», «Футовъ», то есть место, а не человек.
    "bx0000205": "проверено пробой из шести страниц: таблицы поселений, "
                 "жители только счётом",
    # Алфавитный указатель поселений (Новочеркасск, 1915): на каждое —
    # число дворов и десятин, жители счётом, округ, заседательский
    # участок, епархия и благочиние, судебно-следственный участок с
    # окружным судом, мировой, медицинский, ветеринарный и горный
    # участки, учреждения, расстояния, сроки ярмарок, почтовый адрес.
    # Проба из шести страниц: личных имён нет, в графе «учрежденія и
    # лица» стоят сокращения заведений («Сел. пр.», «Кирпичн. зав.»,
    # «вод. мел.»). Фамилию искать не в чем — но справочник рабочий, и
    # им уже пользовались: строки Кочетовской (№ 2025) и Ровенек
    # (№ 3768) выписаны в places.json, разделы «справочник_1915».
    # Нужное место ищется тремя-четырьмя страницами по алфавиту,
    # качать все 665 незачем.
    "bx0000206": "проверено пробой из шести страниц: алфавитные таблицы "
                 "поселений (1915), личных имён нет; как справочник по "
                 "подчинению места использован — см. places.json",
}
...
SKIP_SERIES = [
    ...
    # Соседняя губерния, а не Область войска Донского. В COSSACK стоит
    # голое «Памятная книжка» — оно нужно, чтобы поймать донской ряд
    # целиком, но заодно тянет в очередь книжки любой губернии, и
    # SPRAVOCHNIK ставит их в самое начало второй очереди: шесть
    # ставропольских томов стояли впереди всех донских документов.
    # Обе наши линии внутри области: Кочетовская — 1-й Донской округ,
    # Ровеньки — Миусский (позже Таганрогский). Уйдёт след за пределы
    # области — правило снимается одной строкой, тома никуда не делись.
    (re.compile(r"Памятная книжка Ставропольской губернии", re.I),
     "другая губерния: Ставропольская, а не Область войска Донского; "
     "обе искомые линии внутри области"),
    # Отчёты наказного атамана — 73 тома в каталоге, почти вся вторая
    # очередь. Ряд распадается надвое по году, и два куска отсеяны по
    # разным основаниям, поэтому и правил три. Узкие, «рукописные»,
    # стоят первыми: общее правило ниже поймало бы и их, а причина у
    # них другая.
    #
    # До 1888 года включительно отчёт не печатали: в библиотеке лежат
    # сканы рукописных дел из Государственного архива Ростовской
    # области. Почерк писарский и человеку читается легко, но Tesseract
    # не берёт его вовсе — на пробе в две страницы с тома выходит 5-30
    # «слов» на страницу и ни одного словарного, тогда как печатный том
    # той же серии даёт 130-210. Искать в них этим конвейером нечего:
    # пустой ответ означал бы только то, что распознавание молчит.
    # Проверены пробой все 26 томов; сами дела — сводные ведомости по
    # округам и денежные росписи, имён в них и не ожидается.
    (re.compile(r"Отчет наказного атамана Войска Донского", re.I),
     "рукописное дело из Государственного архива Ростовской области: "
     "OCR не читает писарский почерк (проверено пробой всех 26 томов "
     "рукописной части ряда)"),
    (re.compile(r"Отчет Войскового наказного атамана о состоянии Области "
                r"войска Донского: за 18(?:7[01]|8[0-8]) год", re.I),
     "рукописное дело из Государственного архива Ростовской области: "
     "OCR не читает писарский почерк (проверено пробой всех 26 томов "
     "рукописной части ряда)"),
    # С 1889 года отчёт печатный, и OCR его берёт — но называть в нём
    # некого. Это обзор по войску и области в целом: оскудение казачьего
    # населения, землепользование, недоимки, народное здравие, тюрьмы,
    # конский состав, училища. Единственное имя в томе — подпись атамана
    # под отчётом. Проверено одиннадцатью томами: четыре прочитаны
    # целиком (bv0000382, bv0000383, bv0000384, bv0000385), семь взяты
    # пробой по шесть страниц вразброс — обе части, военная и
    # гражданская, оба конца ряда и оба личных отчёта: bv0000482,
    # bv0000510, bv0000739, bv0000514, bv0000487, bv0000711, bv0000712.
    # На 150 тысяч знаков ни одной фамилии: всё, что похоже на неё, —
    # родительный падеж из заголовков таблиц («Генераловъ», «Офицеровъ»,
    # «Полковъ», «Участковъ», «Раскольниковъ») либо искажения OCR.
    # Названия мест в отчётах есть, и наши среди них встречаются
    # (Кочетовский перекат на Дону в томе за 1912 год), но это место,
    # а не человек.
    (re.compile(r"Отчет Войскового наказного атамана о состоянии Области "
                r"войска Донского", re.I),
     "печатный отчёт по войску и области в целом: частных лиц не "
     "называет, единственное имя в томе — подпись атамана "
     "(проверено одиннадцатью томами ряда)"),
]
...
def classify(title: str) -> str:
    ...
    дорог нет. По той же причине раньше `SKIP` стоит `NAMED_LISTS`:
    адрес-календарь служащих — это перечень имён, и правило о теме
    (железные дороги) не должно решать его судьбу.
    Газета стоит там же, где приказы, — раньше `REGION`:
